...
import type { GameType, Game } from '@/types/models'
...
<template>
  <div class="points-wrapper">
    <boom-board>
      <template v-if="pointsStore.currentSeason.needPasscard">
    ...
    <div class="points-body">
      <div class="nav">
        <div class="left">
          <div
            class="icon"
            :class="[
              `icon-${item.overwolfId === '24504' ? 'wukong' : item.name.toLowerCase()}`,
              gameType === (item.overwolfId === '24504' ? 'WUKONG' : item.name) ? 'active' : ''
            ]"
      ...
      <div
        ...
        :class="{
          apex: gameType === 'APEX',
          lol: gameType === 'LOL',
          wukong: gameType === 'WUKONG'
        }"
      >
        <div class="bg-wrapper">
          <template v-if="gameType === 'WUKONG'">
            <div class="button">Game Data Mining</div>
            <div class="game-info">
              <div class="item">
                <svg-icon name="home-skill-damage"></svg-icon>
                <div class="label">Mining Time</div>
                <div class="score">
                  <boom-number :number="pointsStore.myGameInfo.miningTime" unit="minutes" />
                </div>
              </div>
              <div class="item">
                <svg-icon name="home-champion-kills"></svg-icon>
                <div class="label">Mining Rewards</div>
                <div class="score">
                  <boom-number :number="pointsStore.myGameInfo.miningTime" unit="GPTS" />
                </div>
              </div>
            </div>
          </template>
            ...
            <boom-button-group v-model="checkedType" :options="buttonOptions">
              <template #ingame>
                ...
                <boom-tip title="About In-Game Challenges">
                  ...
                  <p>
                    Note: Points from in-game challenges are scored individually per match and
                    awarded to Boomers upon completion.
                  </p>
                </boom-tip>
              </template>
              <template #card>
                ...
                <boom-tip title="About Challenge Card">
                  ...
                  completion criteria, offering larger Gamer Point rewards. These cards are graded
                  as S\A\B\C, with higher grades providing richer rewards. Each day, you have
...
.points-wrapper {
  ...
  .points-body {
    ...
    .nav {
      ...
      .icon-wukong {
        background-image: url('@/assets/images/icon-wukong.svg');
        width: 32px;
        height: 20px;

        &:hover {
          background-image: url('@/assets/images/icon-wukong-hover.svg');
        }

        &.active,
        &.active:hover {
          background-image: url('@/assets/images/icon-wukong-active.svg');
        }
      }
      .icon-lol {
    ...
    .card {
      ...
      &.wukong {
        background-image: url('@/assets/images/bg_wukong.png');
      }
