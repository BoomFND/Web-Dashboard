...
const handleClick = async (item: SharingTask) => {
  ...
  try {
    ...
    if (item.code === 'share_video' && item.action) {
      ...
      const redirectUrl = res.redirectUrl
      window.open(redirectUrl, '_blank')
    } else if (item.code === 'join_telegram') {
      if (isSubmitting.value) return
      isSubmitting.value = true
      await questStore.followTelegram()
      setTimeout(() => {
        isSubmitting.value = false
      }, 1000)
      if (item.redirectUrl) window.open(item.redirectUrl, '_blank')
    } else if (item.code === 'follow') {
      if (isSubmitting.value) return
      isSubmitting.value = true
      await questStore.followTwitter()
      setTimeout(() => {
        isSubmitting.value = false
      }, 1000)
  ...
  <div class="share-card">
    <div class="card">
      <div class="left">
        <div class="wrapper">
          <div class="title">
            <span class="text">Daily Sharing Quest</span>
